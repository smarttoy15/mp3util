...
ID3v2_tag* load_tag(const char* file_name)
{
    ...
    char *buffer = (char*) malloc((ID3_HEADER + header_size) * sizeof(char));
    if(buffer == NULL) {
        perror("Could not allocate buffer");
        fclose(file);
        return NULL;
    }
    //fseek(file, ID3_HEADER, SEEK_SET);
    fread(buffer, header_size + ID3_HEADER, 1, file);
    fclose(file);


    //parse free and return
    ID3v2_tag *tag = load_tag_with_buffer(buffer, header_size + ID3_HEADER);
    free(buffer);
...
ID3v2_tag* load_tag_with_buffer(char *bytes, int length)
{
    ...
    if(tag_header->tag_size > (length - ID3_HEADER))
    {
    ...
    memcpy(tag->raw, bytes + ID3_HEADER, tag->tag_header->tag_size * sizeof(char));
...
void set_tag(const char* file_name, ID3v2_tag* tag)
{
    ...
    fseek(file, old_size + ID3_HEADER, SEEK_SET);
    int c;
    while((c = getc(file)) != EOF)
    {
        putc(c, temp_file);
